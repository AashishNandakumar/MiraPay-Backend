...
class AddData(APIView):
    def post(self, request):
        try:
            ...
            serialized_financial_info = serializers.FinancialInformationSerializer(
                data=financial_info
            )
            ...
            return Response(
                {"message": "data saved successfully"}, status.HTTP_201_CREATED
            )
...
class GenerateUUID(APIView):
    def get(self, request):
        try:
            ...
            return Response(
                {"error": "Error in generating UUID"},
    ...
    def get(self, request):
        ...
        try:
            presigned_url = s3Client.generate_presigned_url(
                "put_object",  # permission on the bucket
                Params={
                    "Bucket": bucket_name,
                    "Key": object_name,
                ...
                ExpiresIn=3600,
            )  # 1 hr expiry time for the URL

            return Response({"url": presigned_url}, status.HTTP_200_OK)

        except (
            NoCredentialsError
        ):  # trigger when the backend has no proper cred. to interact wit AWS
            return Response(
    ...
    def post(self, request):
        try:
            ...
            if face_matches:
                ...
                matched_external_image_id = match["Face"]["ExternalImageId"].split(".")[
                    0
                ]
                ...
                return Response(
                    {"userId": matched_external_image_id}, status.HTTP_200_OK
                )
            else:
            ...
            return Response(
                {"Error": "Failed in fetching User Facial Data"},
                status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class UserData(APIView):
    def get(self, request):
        try:
            ...
            financialInfo = get_object_or_404(
                models.FinancialInformation, userId_id=user_id
            )
            ...
            if serializer1 and serializer2:
                return Response(
                    {"userInfo": serializer1.data, "financialInfo": serializer2.data},
                    status.HTTP_200_OK,
                )
            else:
                return Response(
            ...
            return Response(
                {"Error": "Failed in fetching User Data"},
                status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class BillUserAndSendReceipt(APIView):
    ...
    def post(self, request):
        try:
            ...
            )  # get `user_upi_id` and `user_email_id` through `financial_information`

            # TODO: Generate UPI payment link and send it to the user(sms)
            payment_link = payments.generate_payment_link(
            ...
            if invoice_pdf:
                emails.send_email_with_pdf_attachment(
                    invoice_pdf, financial_information
                )
                serializer.save()  # save only after the payment is successful.
            else:
                return Response(
                    {"Error": "Invoice Pdf not received"}, status.HTTP_400_BAD_REQUEST
                )

            return Response(
                {"Success": "Successfully  stored Invoice information"},
                status.HTTP_201_CREATED,
            )
        except Exception as e:
            print("Error while Billing User: ", e)
            return Response(
                {"Failed": "Failed to store Invoice information"},
